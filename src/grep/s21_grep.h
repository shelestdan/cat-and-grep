...
void Read_file(char **argv, int argc, int optind, options *flags, struct line_const); //чтение файла
void Flags(char f, char **argv, options *flags, struct line_const);                   // выбор флага
void pattern(char **argv, options *flags, struct line_const);
void kolizz(FILE *fileop, int argc, char *p, options *flags, struct line_const); //функция отработки флагов
